...
// Mint a pageant-nft collectible to the buyer via admin_mint (platform-signed). Called AFTER the
// buyer's USDC payment clears, so the buyer only signs the payment. Enforces one-per-wallet on-chain.
export async function mintPageantNft(params: { toAddress: string; candidateId: number }): Promise<MintResult> {
  if (MODE === "live") {
    const contractId = requireEnv("PAGEANT_NFT_CONTRACT_ID");
    const { txHash, returnValue } = await invoke(contractId, "admin_mint", (sdk) => [
      new sdk.Address(params.toAddress).toScVal(),
      sdk.nativeToScVal(params.candidateId, { type: "u32" }),
    ]);
    return { tokenId: String(returnValue ?? ""), txHash, mode: "live" };
  }
  return { tokenId: fakeTokenId(), txHash: fakeTxHash(`pageant-nft:${params.candidateId}`), mode: "mock" };
}
...
// ---------------------------------------------------------------------------
// Prediction market (deployed escrow-pool contract)
// ---------------------------------------------------------------------------
// Admin calls (create/resolve/close/cancel) are platform-signed via invoke().
// Stakes and claims move a fan's USDC, so the FAN signs them in Freighter — we build
...
export async function buildStakeTx(params: { fanAddress: string; marketId: number; option: number; amountUsdc: number }): Promise<{ xdr: string; txHash: string }> {
  ...
  const op = contract.call(
    ...
    sdk.nativeToScVal(params.marketId, { type: "u32" }),
    sdk.nativeToScVal(params.option, { type: "u32" }),
    sdk.nativeToScVal(BigInt(usdcToBaseUnits(params.amountUsdc)), { type: "i128" })
  );
  let tx = new sdk.TransactionBuilder(source, { fee: sdk.BASE_FEE, networkPassphrase: passphrase })
    .addOperation(op)
    .setTimeout(180)

...
export function SeatMap({
  ...
  const [, setSelectedId] = useState<string | null>(initialSelectedSeatId || null);
  const [hoveredId, setHoveredId] = useState<string | null>(null);
  const [visible, setVisible] = useState(false);

  // Stable dependency key so a fresh array literal doesn't re-run the effect endlessly.
  const takenKey = (takenSeatIds ?? []).join(",");

  useEffect(() => {
    const taken = new Set(takenSeatIds ?? []);
    const raw = generateSeats().map((s) => {
      if (s.id === initialSelectedSeatId) return { ...s, status: "selected" as SeatStatus };
      if (taken.has(s.id)) return { ...s, status: "taken" as SeatStatus };
      return s;
    });
    setSeats(raw);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [initialSelectedSeatId, takenKey]);

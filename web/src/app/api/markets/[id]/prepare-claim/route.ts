...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (!["resolved", "cancelled"].includes(market.status)) return NextResponse.json({ error: "not_resolved" }, { status: 409 });
  if (!marketConfigured() || market.chainMarketId == null) return NextResponse.json({ error: "not_onchain" }, { status: 409 });

  const refundable = market.status === "cancelled";
  const eligible = await db.prediction.findFirst({
    where: refundable
      ? { marketId: id, fanId: auth.fanId, status: { in: ["active", "lost"] } }
      : { marketId: id, fanId: auth.fanId, option: market.winningOption ?? -1, status: "won" },
  });
